...
val_dataloader = dict(
    dataset=dict(
        ...
        data_root=data_root,
        metainfo=metainfo,
        ann_file=data_root + 'ms_coco-format_labels/fold1_test.json',
        data_prefix=dict(img=data_root + 'all/images/')
        ))
test_dataloader = val_dataloader

val_evaluator = dict(  # Validation evaluator config
    type='CocoMetric',  # The coco metric used to evaluate AR, AP, and mAP for detection and instance segmentation
    ann_file=data_root + 'ms_coco-format_labels/fold1_test.json',  # Annotation file path
    ...
    outfile_prefix='./work_dirs/test')  # 要保存的 JSON 文件的前缀
